                         ...
                         <div style={{display:'flex',flexDirection:'column',gap:8}}>
                           ...
                           <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                             <div>
                               ...
                               <input id={`phase-start-${p.id}`} className={"phase-start " + (phaseErrors[p.id]?.start || phaseErrors[p.id]?.order ? 'invalid' : '')} type="date" min={todayIso} value={p.start} onChange={(e)=>updatePhase(p.id,{start:e.target.value})} onClick={()=>{ const el = document.getElementById(`phase-start-${p.id}`); if(el){ if(typeof el.showPicker === 'function'){ el.showPicker(); } else { el.focus(); } } }} style={{width:'100%',padding:10,borderRadius:8,border: phaseErrors[p.id]?.start || phaseErrors[p.id]?.order ? '1px solid #ef4444' : '1px solid #e7e9ee',background:'#fff'}} />
                             </div>
                             <div>
                               <label style={{fontWeight:700}}>End Date <span style={{color:'#ef4444'}}>*</span></label>
                               <input id={`phase-end-${p.id}`} className={"phase-end " + (phaseErrors[p.id]?.end || phaseErrors[p.id]?.order ? 'invalid' : '')} type="date" min={p.start || todayIso} value={p.end} onChange={(e)=>updatePhase(p.id,{end:e.target.value})} onClick={()=>{ if(!p.start) return; const el = document.getElementById(`phase-end-${p.id}`); if(el){ if(typeof el.showPicker === 'function'){ el.showPicker(); } else { el.focus(); } } }} readOnly={!p.start} title={!p.start ? 'Set start date first' : ''} style={{width:'100%',padding:10,borderRadius:8,border: phaseErrors[p.id]?.end || phaseErrors[p.id]?.order ? '1px solid #ef4444' : '1px solid #e7e9ee',background: !p.start ? '#f3f4f6' : '#fff'}} />
                             </div>
                           </div>
